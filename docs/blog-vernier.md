...
Description: Build AI sells egocentric factory video with a quality number produced by a single VLM pass and no interval. I measured it against human gold: every judge error inflates the statistic, one of the two published margins does not survive correction, and the interval is still too wide to settle it.
Draft: true
...
| **Gold** | 93 frames, one rater, written rubric, 64 minutes of labelling |
...
| **Judge error** | 22 errors on 93 frames, every one inflating the statistic |
| **The margin** | +6.62pp over EPIC becomes -4.69pp [-18.00, +8.63], unresolved |
| **Cost** | about $9 per 10,000 frames, no API vendor required |

The published number is not wrong in any way I can show. It is a point estimate with no interval, produced by a judge that over-reports on every frame where it disagrees with a human, and the honest range for the headline figure on the batch you would actually buy runs from 70% to 92%.
...
The oracle is a person, not a model. I wrote a [rubric](https://github.com/caiotheodoro/vernier/blob/main/docs/RUBRIC.md) with twelve numbered rules resolving what their prompt leaves open (gloves, reflections, motion blur, an idle grip that is not manipulation), then labelled 93 frames balanced across the three corpora: 33 Egocentric, 30 Ego4D, 30 EPIC-KITCHENS. Median ten seconds a frame, 64 minutes in total. The labelling tool has no read path to judge output; the rater cannot see what the model said, before or during a pass, and that is enforced in code rather than by discipline.

The statistics were chosen before any frame was drawn and frozen in a [pre-registration](https://github.com/caiotheodoro/vernier/blob/main/docs/PRE-REGISTRATION.md): [PPI++](https://arxiv.org/abs/2301.09633) for prevalence, so the interval stays valid however biased the judge turns out to be; [Gwet's AC1](https://arxiv.org/abs/2606.00093) as the headline agreement statistic with Cohen's kappa reported beside it, because kappa is uninformative at 96% base rates; a cluster bootstrap over worker id, B = 10,000, wherever a worker id exists. Every deviation from that document is a dated entry in a decision log that now runs to 82 entries.
...
flowchart TD
  ...
  H["Human gold<br/>93 labels, rubric 1.2.0"]
...
I do not know why, and the honest reading cuts against me as much as against them. My own gold set shows this judge over-counting hands: it calls two hands on 62 of 93 frames where the rater counted 53. So the parsimonious explanation for a high two-hands rate is my judge, not their number. What keeps it interesting is that the same figure carries the largest design effect on both corpus draws, measured further down, and that three separate measurements single out the same column. Three measurements pointing at one figure, and none of them explaining it, is worth reporting and is not worth over-claiming.

## What the human labels changed

**Every judge error in the gold set inflates the statistic.** This is the finding I did not expect and the one that survived everything else in this post. Across 93 frames the open judge makes 22 errors against the rater, and all 22 run the same way.

| task | judge over-reports | judge under-reports |
|---|---:|---:|
| hand count | **14** | 0 |
| active manipulation | **8** | 0 |

Zero under-reports on either task. The at-risk denominators matter here, because a frame the rater scored as two hands cannot be over-counted: 14 of the 40 frames that could be over-counted were, and 0 of the 72 that could be under-counted were. On manipulation it is 8 of 27 against 0 of 66. This is not a subtle asymmetry that needs a test to see.

It also is not a quirk of the model I chose. Build AI's own stored `gemini-2.5-flash` labels, a different family from a different lab, carry the same sign on the same 93 frames. Two judges, one direction.
...
{"type":"forest",
 ...
 "panels":[
  {"metric":"active manipulation","rows":[
    ...
    {"label":"Egocentric-10K, PPI++","point":0.8084,"ci":[0.701,0.9158],"color":1},
    {"label":"EPIC-KITCHENS-100, published","point":0.8504,"color":"neutral","note":"no interval published"},
    {"label":"EPIC-KITCHENS-100, PPI++","point":0.8552,"ci":[0.7766,0.9339],"color":1},
  ...
  {"metric":">=1 hand","rows":[
    ...
    {"label":"Egocentric-10K, PPI++","point":0.9272,"ci":[0.8609,0.9934],"color":1},
    {"label":"EPIC-KITCHENS-100, published","point":0.9037,"color":"neutral","note":"no interval published"},
    {"label":"EPIC-KITCHENS-100, PPI++","point":0.8605,"ci":[0.7641,0.9568],"color":1},
 ...
 "caption":"n_gold is 33 on the Egocentric arm and 30 on each of the others, against 200 judged frames per arm. Every published mark on this chart is a point with no width, and that is the mark being sold."}
```

The Egocentric manipulation rate, the headline, corrects from a published 91.66% to 80.84% with a 95% interval of [70.10, 91.58]. The published value sits at the top edge of that interval rather than outside it. The interval is wide because 33 gold frames on that arm is not many, and it is a lower bound on the true width because the evaluation release ships no worker id to cluster over.
...
{"type":"forest",
 ...
 "panels":[
  {"metric":"margin in percentage points","rows":[
    ...
    {"label":"manipulation, corrected","point":-4.69,"ci":[-18.0,8.63],"color":0},
    {"label":">=1 hand, published","point":6.05,"color":"neutral","note":"no interval published"},
    {"label":">=1 hand, corrected","point":6.67,"ci":[-5.03,18.37],"color":1}]}],
 "caption":"The manipulation margin changes sign under correction and is the only one of four that does. Its interval still covers the published value, so this fails to resolve the published margin rather than refuting it. The hand-visibility margin barely moves."}
```

The manipulation lead changes sign, from +6.62pp to -4.69pp. It is the only one of the four margins I computed that does. And the corrected interval still covers the published value, so I cannot refute that margin; I can only say the data does not settle it. This project's own [red team](https://github.com/caiotheodoro/vernier/blob/main/docs/RED-TEAM.md) wrote the rule for this case before any result existed: a wide interval containing the null is underpowered, not null, and treating the two as the same thing is the exact error the audit exists to object to. A headline claiming the margin reverses is not available on this data, and I drafted one before I checked.

The hand-visibility margin behaves differently. Published +6.05pp, corrected +6.67pp, barely moved. One of the two comparative claims survives correction for judge error and the other does not, which is worth knowing before relying on either.

The useful part is that this prices its own completion. Holding the point estimate, about 42 gold frames per arm would put the published margin outside the corrected interval, against the 33 and 30 that exist. Twenty-five more labelled frames would settle a number that is currently sold without one. Not a second corpus, not a better judge, not more compute.
...
**Domain bias reversed.** Judge error on manipulation is 9.09% on Egocentric frames and 3.33% on EPIC-KITCHENS, the opposite of the prediction, on arms of 33 and 30. My own power analysis, run before any labelling, called this underpowered even at the full pre-registered size, so a reversed result here is ambiguous rather than informative. There is also a live confound I disclosed before the data existed and never probed: the judge has plausibly seen EPIC-KITCHENS and Ego4D in pretraining and cannot have seen the gated Egocentric corpus, which pushes accuracy in the opposite direction from the prediction. A null here has three readings and I can rule out none of them.
...
**Calibration is degenerate by construction.** Expected calibration error is 0.1505 on hand count and 0.0860 on manipulation, but 92 of 93 frames land in a single confidence bin under greedy decoding. That number describes the decoding strategy more than the judge, and reporting it as a calibration result would be misleading.

## Honest limits

One rater, 93 frames, 64 minutes of labelling, and no inter-rater agreement statistic anywhere in this project, because there is one person. The blind re-label is a weaker substitute and, as run, weaker again: a median of 2.4 hours, not the seven days specified. The rater wrote the rubric after reading Build AI's prompt, which is anchoring and is not removable at n=1. The two judge arms share plausible pretraining lineage, so agreement between them is an upper bound on reliability rather than a measure of it. The design effect is measured on my corpus draws and never on the vendor's evaluation frames, and nothing can measure it there until a grouping variable ships. The margin estimand is exploratory, not pre-registered, and its interval covers the published value. Half the advertised SLA, the pose accuracy claim, needs annotations no public release ships and is untouched here. Whether hand visibility predicts training value, which is the premise of the entire metric, is untested and now will not be. Every interval on the evaluation arms is iid and labelled as a lower bound on its true width. All 22 judge errors come from 93 frames, and 22 events is a small number to carry a directional claim.
