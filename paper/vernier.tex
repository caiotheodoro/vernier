...
%
% arXiv categories: primary stat.AP, cross-list cs.CV (D089). This is a measurement
% paper about a corpus statistic, not a vision-method paper; the only vision artefact is a
% linear probe reported as a negative result.
...
\begin{abstract}
\noindent
Commercial dataset vendors advertise corpus quality with statistics produced by a
...
91.66\% show active manipulation, both ahead of Ego4D \\citep{grauman2022ego4d} and EPIC-KITCHENS-100
\\citep{damen2022epic100}, from 10{,}000
frames labelled once with no human label, no interval, and no test that the labeller scores the
compared corpora on the same scale. We pre-registered a protocol, replicated the figures with an
independent open-weights judge on two releases a year apart, collected \vernierNPrimary{} human
labels from one rater against a written rubric, and re-estimated the published quantities with
prediction-powered inference. Three findings. First, judge error is strongly asymmetric: on the
hand-visibility indicator the published figure reports, the judge over-calls 6 of the 24 frames
where over-calling was possible and under-calls 0 of 129, and on manipulation 10 of 33 against
2 of 120, both significant at $p < 10^{-5}$. Second, the corpus is clustered by camera wearer,
with a design effect of 1.25 to 1.66 on two independent 10{,}000-frame draws, so an
independent-frames interval understates its width by 11 to 22\%; no interval is published, and
the released frames carry no identifier that would let anyone else compute one. Third, the
comparative claims cannot be resolved at this gold size: correcting both sides of the
manipulation margin against EPIC-KITCHENS-100 moves it from a published $+6.62$ to $-1.02$
percentage points, 95\% interval $[-11.68, +9.65]$, which covers the published value. None of
the pre-registered hypotheses was confirmed, and we report what moved when we collected
more labels. Artifacts and protocol are public; the measurement card reports
\texttt{\vernierVerdict}.
\end{abstract}
...
\begin{enumerate}
\item \textbf{An asymmetric-error result.} On the hand-visibility indicator the published
      figure reports, the judge over-calls 6 of 24 at-risk frames and under-calls none of 129;
      on manipulation, 10 of 33 against 2 of 120. Both are significant at $p < 10^{-5}$. A
      judge-derived prevalence of this kind is better read as an upper bound than as a point
      estimate. We report the full confusion matrix rather than a summary count, because the
      ternary hand count and the indicator are different estimands and only the latter bears on
...
\item \textbf{A design effect for a corpus quality statistic.} Frames from one camera wearer
      ...
      so an interval that treats frames as independent understates its width by 11 to 22\%,
      equivalently the cluster-aware interval is 12 to 29\% wider. We are not
...
survives the correction. We are not aware of prior work that combines those three properties, and the positioning above says which established lines each of them comes from.
...
\paragraph{Auditing a dataset someone else published.}
Re-examining a released corpus with a better human protocol is an established move.
\citet{beyer2020imagenet} re-label the ImageNet validation set and find that reported gains are
``substantially smaller than those reported on the original labels''.
\citet{northcutt2021labelerrors} estimate label-error rates across ten third-party benchmarks
using each benchmark's own released data plus human validation, and show that model rankings
change once the errors are corrected. \citet{raji2019actionable} audit commercial vendors'
published accuracy claims directly and study what the vendors did afterwards.

Our estimand differs from the first two in a way that matters for what an audit can conclude.
Their target is label correctness against a fixed taxonomy, where a wrong label is wrong by
construction and the corrected number is simply better. Ours is a vendor-authored statistic
published as a selling point, with no interval attached and a comparative claim resting on it,
where the question is not whether individual labels are right but how wide the published number
should have been and whether the comparison survives. From \citet{raji2019actionable} we take the
framing that an audit of a commercial claim is a different object from an audit of a benchmark,
and differ in that our subject is a measurement rather than a disparity.
...
ignores. The design effect itself is standard survey sampling \citep{kish1965survey}; our
measurement is that argument applied to a dataset statistic rather than a model score, with the clustering variable being the person wearing the camera.
...
twenty-two numbered rules across the two tasks, resolving what the vendor's prompt leaves
open, such as gloves,
...
\caption{Every pre-registered hypothesis, its threshold, and its outcome. Five fail against
their own criteria, one is null, one reverses direction, and H7 was measured after the
pre-registration predicted it could not be. \textbf{None was confirmed.} H8 and H7 carry no
falsifiable threshold, so the falsifiable slate is seven, of which five failed. The exploratory
row was not pre-registered and is reported as such.}
\label{tab:ledger}
\end{table}
...
\begin{table}[t]
\centering\small
\input{generated/error_direction}
...
Table~\ref{tab:errdir} is the paper's first result, and the estimand matters. The published
figure is the proportion of frames showing \emph{at least one} hand, so the quantity bearing on
it is the binary indicator, not the ternary count. On that indicator the judge over-calls 6 of
the 24 frames where the rater saw no hand, and under-calls none of the 129 where it saw at least
one, which is 25.0\\% with a Wilson 95\\% interval of $[12.0, 44.9]$ against 0.0\\%,
$[0.0, 2.9]$. On manipulation it is 10 of 33 against 2 of 120, 30.3\\% $[17.4, 47.3]$ against
1.7\\% $[0.5, 5.9]$. Fisher's exact test gives $p < 10^{-5}$ on both. We give intervals on these
counts because a paper objecting to a bare number should not report one.

A further 10 disagreements are one-hand-against-two confusions. Those are errors on the ternary
count and no error at all on the published indicator, and they bear instead on the two-hands
figure, which Section~\ref{sec:repl} attributes to our own judge rather than to the vendor.
A single summary count would fold those 10 into the headline; the full matrix keeps them
separate.
...
The vendor's own stored labels show the same asymmetry more strongly, and on the estimand
that matters most, since those labels are what the published figure was computed from. On the
$\\geq$1-hand indicator the vendor's judge over-calls \\textbf{9 of 24} at-risk frames, 37.5\\%
with a Wilson 95\\% interval of $[21.2, 57.3]$, and under-calls \\textbf{0 of 129},
$[0.0, 2.9]$; Fisher's exact test gives $p \\approx 1.3\\times10^{-8}$. The open judge we
substituted is the weaker of the two. So the asymmetry is not an artifact of the model we chose,
and the audited figure is the one it bites hardest. It is not uniform across cells: pooled over
the ternary hand count and manipulation the vendor is 25 over-reports against 7, where the open
judge is 26 against 2, and on the EPIC-KITCHENS manipulation cell the vendor reverses.

\section{Corrected prevalence, and the comparative claim}

\begin{table}[t]
\centering\small
\input{generated/prevalence}
...
\begin{figure}[t]
\centering
\input{generated/fig_forest}
\caption{Active-manipulation prevalence per corpus. The vendor's published figure is a bar with
no width, because none is published. The judge-only rate is what a single labelling pass
produces. The gold-corrected estimate carries a 95\% interval, which on the Egocentric arm spans
roughly fourteen percentage points. The argument of this paper is the difference between the
first mark and the third.}
\label{fig:forest}
\end{figure}
...
(Table~\ref{tab:prev}, and Figure~\ref{fig:forest}).
...
\subsection{The same correction with the vendor's own judge}
\label{sec:vendorarm}

The estimates above use our open judge as the model arm, over the frames left in a 200-frame
gold pool. The release also ships the vendor's own labels for 9{,}800 frames per corpus, and
those are disjoint from our gold frames, which is what prediction-powered inference requires.
Using them makes the model arm the judge that produced the published figure, and the estimand
becomes the published number corrected for its own error rather than a substitute's.

\begin{table}[h]
\centering\small
\input{generated/vendor_prevalence}
\caption{Gold-corrected prevalence with the vendor's own stored labels as the model arm over the
full evaluation arms. The unlabelled sample rises from about 140 frames to 9{,}800, which
collapses the unlabelled variance term to almost nothing and leaves the gold sample as the only
material constraint.}
\label{tab:vendorprev}
\end{table}

The manipulation margin against EPIC-KITCHENS-100 becomes $-1.32$ percentage points with a 95\%
interval of $[-10.78, +8.14]$, so the sign and the conclusion are unchanged and the interval is
about 11\% narrower. What changes materially is the price of resolving it. With the unlabelled
term almost gone, separating the corrected margin from the published value needs about 89 gold
frames per arm, against the 63 and 60 that exist. The earlier figure of 226 was an artifact of
having drawn a 200-frame judged pool when the vendor had already labelled 9{,}800.


\subsection{Does the second batch of labels change the answer?}
\label{sec:sensitivity}

Section~\ref{sec:moved} reports that 60 of the \vernierNPrimary{} labels were collected after
the first results were public, at a median of three seconds a frame against ten for the first
batch. Disclosing that without testing it would leave the reader to guess, so we report the
headline counts on three subsets.

\begin{table}[h]
\centering\small
\input{generated/sensitivity}
\caption{Judge error against the rater on the first 93 labels, on all \vernierNPrimary{}, and on
the \vernierNPrimary{} minus every label recorded in two seconds or less.}
\label{tab:sens}
\end{table}

The over-report rate is stable: 5 of 21 at-risk frames on the first batch, 6 of 24 on the full
set, and 6 of 24 with the fastest labels removed. What moves is the pair of manipulation
under-reports, and they move in an informative direction. The first batch contains none, the
full set contains two, and removing labels recorded in two seconds or less removes one of them.
The asymmetry we report is therefore not created by the second batch; the two exceptions to it
are, and they sit among the fastest labels in the study. We report the asymmetry as strong
rather than absolute for that reason, and a reader who discounts the fast labels entirely gets a
stronger version of our claim, not a weaker one.
...
were independent understates its width by 11 to 22\%, equivalently the cluster-aware interval
is 12 to 29\% wider.
...
\paragraph{Buying labels made the vendor's number look better, and the price we quoted was
wrong twice.} Having priced the unresolved margin at roughly 42 gold frames per arm, we collected 60
more. The corrected margins moved toward the published figures rather than away and the
asymmetric-error result weakened from absolute to 26 of 28. The price itself was then found to
be wrong: it scaled the whole standard error as $1/\\sqrt{n}$, when only the gold term shrinks
with more labels and the unlabelled term is a floor. Corrected, the manipulation margin needs
about 226 gold frames per arm rather than 59. That figure was then itself superseded: it
assumed the 200-frame judged pool we had drawn, and using the vendor's full 9{,}800-frame arms
(Section~\ref{sec:vendorarm}) brings it to about 89. Two
...
We did not contact the vendor before publishing. That was a pre-registered choice rather than an
oversight: publishing first keeps the work independent rather than negotiated, and the artifact
is built so that either outcome is useful to them, a confirmation with intervals they did not
have or a problem found before a customer finds it. The decision recorded a reversal condition,
that a finding turning out to be a live security or privacy exposure rather than a measurement
question would warrant private disclosure first. Nothing here is one.

The author is also the rater. That matters for reading Section~\ref{sec:moved}, where the second
batch of labels was collected by someone who knew which direction would move the margin, and it
is why Section~\ref{sec:sensitivity} reports the results without those labels. The author has an
interest in the vendor whose work is measured being aware of it. That interest is contained
structurally rather than by assertion: the protocol, hypotheses and stopping rules were frozen
before any result was seen, so no finding could be shaped by how it would be received.

The protocol, the decision log, every result file and the labels are public:
code and protocol at \url{https://github.com/caiotheodoro/vernier}, the labels, membership,
judge output and result files at \url{https://huggingface.co/datasets/caiotheodoro/vernier},
the distilled probe at \url{https://huggingface.co/caiotheodoro/vernier-rung1-probe}, and a
frame-by-frame view of every judge--rater disagreement at
\url{https://huggingface.co/spaces/caiotheodoro/vernier}. A longer, less formal account is at
\url{https://caio.theodoro.dev/blog/vernier-judge-errors-run-one-way}.

The measurement card reports \texttt{\vernierVerdict} with \vernierNClaims{} claims because one
pre-registered item could not be checked at all: a matched three-corpus transfer probe, blocked
because EPIC-KITCHENS-100 registration requires an institutional email this author does not have
and the evaluation release ships no downstream-task labels. That the blocking reason is access
rather than method is itself an instance of this paper's argument. This paper was built from commit \texttt{\vernierGitRev} against
...
unstated width. On this corpus the cluster-aware interval is 12 to 29\% wider than an independent-frames
...
not survive correction cleanly and does not fail either. Settling it does not need a better
judge. It needs about 89 gold frames per arm on the
manipulation comparison, against the 63 and 60 that exist, once the vendor's own full-size
labelled arms are used as the model arm rather than the 200-frame pool we first drew. That is
roughly 55 more labelled frames. It is a step the vendor could have taken before publishing the
number, at no marginal labelling cost beyond what it had already spent.

\appendix
\section{Corrections to our own reported claims}
\label{app:deviations}

Every departure from the frozen protocol, and every correction to a number this project had
already published. The decision log holds the full record; this table is the subset that changed
something a reader would rely on.

\begin{tabular}{@{}llp{0.52\linewidth}@{}}
\toprule
id & date & what changed \\
\midrule
D042 & 2026-09-01 & The pre-registered judge became unreachable for new API keys. H1 was
redefined from a live replication into a comparison against the vendor's stored labels. \\
D057 & 2026-09-03 & Human gold cut from 600 primary and 100 retest to 93 and 30. Every interval
downstream is correspondingly wider. \\
D058 & 2026-09-03 & The retest was redrawn from the rater's own primary pool, because the
original draw overlapped it on only four frames. \\
D076 & 2026-09-05 & The blind re-label was specified at seven days and ran at a median of 2.4
hours, with 0 of 34 pairs meeting the rule. The agreement figures are unchanged; what they
license is narrower. \\
D078 & 2026-09-05 & Seven labels violated a rubric rule nothing enforced. Correcting them moved
judge--human AC1 on manipulation from 0.899 to 0.863, moved two prevalence estimates, and
removed a clean zero-error result on the EPIC arm. \\
D082 & 2026-09-05 & A coverage table reported the cross-domain experiment as tested. Its model
was never fitted. \\
D085 & 2026-09-05 & Sixty labels added after results were public, taking $n$ from 93 to 153.
Both corrected margins moved toward the published values, the price of separating them rose from
42 to 59 gold frames per arm on a calculation later found wrong (D088), and the error asymmetry
weakened from absolute to 26 of 28. \\
D087 & 2026-09-05 & Two errors in this paper's own first draft: the headline asymmetry was
computed on the ternary hand count rather than on the indicator the published figure reports,
and the design-effect width was stated as 12--29\% too narrow when that figure is how much wider
the cluster-aware interval is. \\
\bottomrule
\end{tabular}

\section{Citation verification, including our own errors}
\label{app:citations}

Twenty references were opened at the source in this project's literature pass, and every
bibliography entry here was re-resolved against arXiv before it was written. Eight attributions
were wrong and are recorded so that a reader can weigh the rest.

\begin{tabular}{@{}lp{0.62\linewidth}@{}}
\toprule
reference & what was wrong \\
\midrule
arXiv:2607.02417 & Cited for a 91.3\% human-agreement figure on egocentric video. It is a
camera-motion generation paper with no judge validation and no such figure. Refuted. \\
arXiv:2503.05965 & Cited as prior support for the prompt-sensitivity hypothesis. Its
rating-indeterminacy examples are toxicity, factuality, helpfulness and relevance. Refuted. \\
arXiv:2408.15204 & Cited as the prediction-powered inference paper. It is Confidence-Driven
Inference; PPI is arXiv:2301.09633. Refuted as a PPI citation, retained as CDI. \\
arXiv:2206.01670 & Claimed to support a wider benchmark suite than it does. Downgraded. \\
arXiv:2309.02423 & Claimed never to compare hand distributions across datasets; it does.
Downgraded. \\
arXiv:2402.04788 & Pairwise agreement understated as 53--71\%; the paper reports 72--79\%.
Downgraded. \\
arXiv:2502.04144 & Best-two annotator merging applies only above an IoU threshold. Downgraded. \\
arXiv:2606.00093 & Cited as the source of Gwet's AC1. It is an agreement-reporting checklist and
does not mention AC1; the statistic is Gwet (2008). Found while preparing this paper. \\
\bottomrule
\end{tabular}

\medskip\noindent
Two figures repeated in secondary sources, a ``16.8\,pp prevalence spread from prompt choice''
and a ``19\% to 54\% neutral'' shift, could not be attributed to any source and are cited
nowhere in this work.

\section{Confusion matrices, per corpus and judge}
\label{app:confusion}

\smallskip
{\footnotesize\input{generated/confusion_per_corpus}}

\medskip\noindent
The vendor's matrices are analysis this project had not previously published. Note the
EPIC-KITCHENS manipulation cell, where the vendor's judge under-reports more often than it
over-reports, against the pooled direction.

\section{Variance decomposition of the corrected estimates}
\label{app:ppivar}

\smallskip
{\small\input{generated/ppi_variance}}

\medskip\noindent
$\lambda$ is the control-variate coefficient, estimated on the gold sample and clipped to
$[0,1]$ by the reference implementation's convention. The gold term dominates in every cell, at
55 to 80\% of total variance, so the binding constraint on these intervals is human labels
rather than judge labels. That is the quantitative form of the finding in
Section~\ref{sec:moved}: collecting 60 more labels narrowed the intervals without resolving the
comparison, and raised the estimated number needed to resolve it.

\section{Cluster structure and the implied ICC}
\label{app:clusters}

\smallskip
{\small\input{generated/cluster_detail}}

\medskip\noindent
The design effect is not transferable on its own, because it depends on the mean cluster size
through $\text{deff} \approx 1 + (\bar{m} - 1)\rho$. The implied intra-cluster correlation is the
transferable quantity, and a vendor draw with a different frames-per-worker profile would imply
a different design effect from the same $\rho$.

\section{Run and cost ledger}
\label{app:runs}

\smallskip
{\footnotesize\input{generated/run_ledger}}

\medskip\noindent
Cost and per-call latency were not persisted for the eight prompt-sweep passes; only their rates
are committed. The attributable total is therefore a lower bound on total spend.
