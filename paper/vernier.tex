...
the eight pre-registered hypotheses was confirmed, and we report what moved when we collected
...
wrong.} Having priced the unresolved margin at roughly 42 gold frames per arm, we collected 60
...
about 226 gold frames per arm rather than 59, and the hand-visibility margin cannot be separated
by labelling at all, because the floor the 137-frame unlabelled pool imposes already exceeds
what separation would require. Two
...
judge. It needs roughly 226 more human labels per arm on the manipulation comparison, and on
hand visibility it cannot be settled by labelling at any budget, because the 200-frame judged
pool we drew imposes a variance floor above what separation requires. Judging the full
evaluation arms, which the vendor already released and which costs about nine dollars, would
lower that floor. That is the cheapest open step, and it is a step the vendor could have taken
