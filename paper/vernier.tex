...
one. On manipulation it is 10 of 33 against 2 of 120. Fisher's exact test gives $p < 10^{-5}$ on
both.
...
The vendor's own stored labels, from a different model family, are asymmetric in the same
direction on the same frames, though less sharply: 25 over-reports against 7 under-reports
pooled across both tasks, where the open judge is 26 against 2. The agreement is not uniform. On
the EPIC-KITCHENS manipulation cell the vendor's judge reverses, 1 over-report against 2. So the
asymmetry is not an artifact of the model we chose, and it is also not a property that holds in
every corpus and task.
...
42 to 59 gold frames per arm, and the error asymmetry weakened from absolute to 26 of 28. \\
